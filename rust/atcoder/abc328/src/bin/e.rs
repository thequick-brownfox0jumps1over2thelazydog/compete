...
fn solve() {
    ...
    for indices in (0..M).combinations(N - 1) {
        let edges = UVW
            .iter()
            .enumerate()
            .filter(|(index, uvw)| indices.contains(index))
            .map(|(index, &uvw)| uvw)
            .collect::<Vec<(usize, usize, usize)>>();
        let network = Network::new(NetworkType::Undirected, N, edges.clone());
        let mut histories = vec![false; N];
        network.depth_first_search(0, N, &mut histories, &mut vec![]);

        if histories.iter().all(|&b| b) {
            min_cost = min(
                min_cost,
                edges.iter().map(|&(u, v, w)| w).sum::<usize>() % K,
            )
        };
    }
