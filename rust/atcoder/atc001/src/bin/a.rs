...
fn solve() {
    ...
    // 27 [min]
    // from: 2023-11-14 15:30
    // to: 2023-11-14 15:57
    let mut nodes = vec![];
    let mut start_id = 250_000;
    let mut goal_id = 250_000;
    for i in 0..H {
        for j in 0..W {
            let node_id = i * W + j;
            match C[i][j] {
            ...
            if j < W - 1 && C[i][j + 1] != '#' {
                nodes.push((node_id, node_id + 1, 1));
            }
            if i < H - 1 && C[i + 1][j] != '#' {
                nodes.push((node_id, node_id + W, 1));
            }
        }
    }

    let network = Network::new(NetworkType::Undirected, H * W, nodes);
    let result =
        network.depth_first_search(start_id, goal_id, &mut vec![false; H * W], &mut vec![]);
