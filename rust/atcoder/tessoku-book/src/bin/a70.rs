...
fn solve() {
    ...
    let mut network = Network::new(NetworkType::Directed, 2_usize.pow(N as u32), vec![]);
    for i in 0..network.n_nodes {
        for (x, y, z) in XYZ.iter() {
            let next_node_id = i ^ 1 << x ^ 1 << y ^ 1 << z;
            network.add_edge(i, next_node_id, 1);
        }
    }

    let nodes = network.breadth_first_search(
        A.iter().enumerate().map(|(i, a)| a << i).sum(),
        network.n_nodes - 1,
    );

    println!("{}", nodes[network.n_nodes - 1].distance);
}
