...
fn solve() {
    input! {
        n_nodes: usize,
        n_edges: usize,
        edges: [(Usize1, Usize1, usize, usize); n_edges],
    }
    debug!(n_nodes, n_edges, edges);
    ...
    let mut network = Network::new(
        ...
        edges
            .iter()
            .map(|&(a, b, c, d)| (a, b, c * 10_000 - d))
            .collect::<Vec<(usize, usize, usize)>>(),
    );
    debug!(network);
    let nodes = network.dijkstra(0, n_nodes - 1);
    debug!(nodes);

    let raw_distance = nodes.last().unwrap().distance;
    let floor_distance = raw_distance / 10_000 * 10_000;
    if raw_distance == floor_distance {
        println!("{} {}", raw_distance / 10_000, 0);
        return;
    }

    let ceil_distance = floor_distance + 10_000;
    println!(
        "{} {}",
        ceil_distance / 10_000,
        ceil_distance - raw_distance
    );
}
