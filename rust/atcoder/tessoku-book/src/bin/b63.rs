...
fn solve() {
    ...
    let mut network = Network::new(NetworkType::Undirected, R * C, vec![]);
    for i in 0..R {
        for j in 0..C {
            if YX[i][j] == '#' {
                continue;
            ...
            let current_node_id = i * C + j;
            if YX[i][j + 1] == '.' {
                network.add_edge(current_node_id, current_node_id + 1, 1);
            }
            if YX[i + 1][j] == '.' {
                network.add_edge(current_node_id, current_node_id + C, 1);
            }
        }
    }

    let start_id = SY * C + SX;
    let goal_id = GY * C + GX;
    let nodes = network.breadth_first_search(start_id, goal_id);

    println!("{}", nodes[goal_id].distance);
}
