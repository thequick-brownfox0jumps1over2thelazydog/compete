...
fn solve() {
    ...
    let mut mutable_edges = vec![(-1, -1); Q];
    let mut questions = vec![(-1, -1); Q];
    for i in 0..Q {
        input! {
            query_type: usize,
        }
        ...
        if query_type == 1 {
            ...
            edges[x].2 = true;
            mutable_edges[i] = (edges[x].0 as isize, edges[x].1 as isize);
        } else {
            input! {
                u: Usize1,
                v: Usize1,
            ...
            questions[i] = (u as isize, v as isize);
        }
    }

    let mut union_find = UnionFind::new(N);
    for (a, b, is_mutable) in edges.iter() {
        if !is_mutable {
    ...
    let mut results = vec![];
    for i in (0..Q).rev() {
        if mutable_edges[i].0 >= 0 {
            union_find.unite(mutable_edges[i].0 as usize, mutable_edges[i].1 as usize);
        } else {
            let (u, v) = questions[i];
            results.push(if union_find.same(u as usize, v as usize) {
                1
            } else {
                0
            });
