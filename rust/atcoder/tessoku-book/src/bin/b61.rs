...
fn solve() {
    ...
    let mut max_n_friends = 0;
    let mut max_friends_holder = 0;
    for i in 0..N {
        let n_friends = network.edge_maps[i].len();
        if n_friends > max_n_friends {
            max_n_friends = max_n_friends.max(n_friends);
            max_friends_holder = i + 1;
        }
    }

    println!("{max_friends_holder}");
}
