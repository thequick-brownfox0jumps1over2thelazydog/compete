...
fn solve() {
    ...
    let mut left_hashes = vec![0; N + 1];
    for i in 0..N {
        let x = S[i].to_digit(36).unwrap() as usize;
        left_hashes[i + 1] = modulo_sum(
            x,
            modulo_product(left_hashes[i], RADIX, UPPER_LARGE_PRIME),
            UPPER_LARGE_PRIME,
        );
    }
    debug!(left_hashes);

    let mut right_hashes = vec![0; N + 1];
    for i in 0..N {
        let x = S[N - i - 1].to_digit(36).unwrap() as usize;
        right_hashes[N - i - 1] = modulo_sum(
            x,
            modulo_product(right_hashes[N - i], RADIX, UPPER_LARGE_PRIME),
            UPPER_LARGE_PRIME,
        );
    }
    debug!(right_hashes);
    ...
    for (l, r) in LR.into_iter() {
        let factor = modulo_power(RADIX, r - l + 1, UPPER_LARGE_PRIME);
        let left = (left_hashes[r + 1] + UPPER_LARGE_PRIME
            - modulo_product(left_hashes[l], factor, UPPER_LARGE_PRIME))
            % UPPER_LARGE_PRIME;
        let right = (right_hashes[l] + UPPER_LARGE_PRIME
            - modulo_product(right_hashes[r + 1], factor, UPPER_LARGE_PRIME))
            % UPPER_LARGE_PRIME;
