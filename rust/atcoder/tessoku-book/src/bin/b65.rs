...
fn solve() {
    ...
    debug!(network.edge_maps);

    let mut nodes = network.breadth_first_search(T, N); // NOTE: Nは存在しないノード
    let node_map: HashMap<usize, isize> =
        HashMap::from_iter(nodes.iter().map(|n| (n.id, n.distance)));
    nodes.sort_by(|a, b| b.distance.cmp(&a.distance));
    let bottom = nodes[0].distance;
    debug!(nodes);

    let mut result = vec![0; N];
    for n in nodes.iter() {
        if n.distance == bottom {
            continue;
        }

        result[n.id] = network.edge_maps[n.id]
            .iter()
            .filter(|(next_node_id, edge)| node_map[*next_node_id] > n.distance)
            .map(|(next_node_id, edge)| result[*next_node_id])
            .max()
            .unwrap_or(-1)
            + 1;
        debug!(
            n.id,
            result,
            network.edge_maps[n.id]
                .iter()
                .filter(|(next_node_id, edge)| nodes[**next_node_id].distance > n.distance)
        );
    }
